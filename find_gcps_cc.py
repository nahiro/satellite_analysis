...
if args.ref_multi_band is not None:
    ...
    ref_data = ds.GetRasterBand(args.ref_band+1).ReadAsArray()
...
if args.trg_multi_band is not None:
    ...
    trg_data = ds.GetRasterBand(args.trg_band+1).ReadAsArray()
trans = ds.GetGeoTransform()
...
for trg_indyc in np.arange(args.trg_indy_start,args.trg_indy_stop,args.trg_indy_step):
    ...
    for trg_indxc in np.arange(args.trg_indx_start,args.trg_indx_stop,args.trg_indx_step):
        ...
        if args.ref_data_min is not None and ref_subset_data.min() < args.ref_data_min:
            continue
        if args.ref_data_max is not None and ref_subset_data.max() > args.ref_data_max:
            continue
        ...
        if not args.use_edge:
            if np.abs(p2[0]) >= np.abs(trg_xp_stp*(args.shift_width-0.5)):
                continue
            if np.abs(p2[1]) >= np.abs(trg_yp_stp*(args.shift_height-0.5)):
                continue
